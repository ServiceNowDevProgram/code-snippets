(function process(/*RESTAPIRequest*/ request, /*RESTAPIResponse*/ response) {
    ...
    fieldsGR.addQuery('name', 'sys_user_role'); //querying over particular user table in dictionary record
    fieldsGR.query();
    while (fieldsGR.next()) {
        var fieldName = fieldsGR.element.getDisplayValue();
        if (role1GR[fieldName] && role2GR[fieldName]) {
            var role1Value = role1GR.getDisplayValue(fieldName);  //fetching the value of field for user1
            var role2Value = role2GR.getDisplayValue(fieldName);  //fetching the value of field for user2
